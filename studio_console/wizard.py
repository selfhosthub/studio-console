...
def _write_override_and_nginx(state: "SetupState") -> None:
    """Write nginx/studio.conf (always) and docker-compose.override.yml (multi-replica only).
    ...
    # ------------------------------------------------------------------
    # docker-compose.override.yml — only for multi-replica
    # Parks the base api/ui services, adds numbered replicas.
    # ------------------------------------------------------------------
    if not use_override:
    ...
    override = (
        ...
        "  ui:\n"
        "    profiles: [nginx-lb-disabled]\n"
        "\n" + "".join(api_services) + "".join(ui_services)
    )
    override_path.write_text(override)
    ok(
        f"nginx/studio.conf + docker-compose.override.yml written ({api_replicas} API, {ui_replicas} UI → port {nginx_port})"
    )
...
def wizard(context: str, env_file: Path) -> bool:
    ...
    print(
        "  If you add workers on another machine, they need:\n"
        f"    SHS_API_BASE_URL={worker_api_url}\n"
        f"    SHS_PUBLIC_BASE_URL={worker_api_url}\n"
        "    SHS_WORKER_SHARED_SECRET=<value>  - keep this secret"
    )
    print()
